...
def train_model(model,
    ...
    for epoch in range(num_epochs):
        ...
        for inputs_llava, inputs_oct, labels,_ in tqdm(train_dataloader, position = 0, leave = True):

            # moving tensors to gpu....
            inputs_llava = inputs_llava.to(device)
            inputs_oct = inputs_oct.to(device)
            ...
            #with torch.set_grad_enabled(True):
                outputs = model(llava_x  = inputs_llava,
                                ret_x    = inputs_oct)

                loss = criterion(outputs,
                                 torch.unsqueeze(labels,-1).type(torch.float32),
            ...
            # statistics
            running_loss += loss.item() * inputs_llava.size(0)
            running_corrects += torch.sum(preds == labels)

            if cnt in random_batches_train:
                random_collection_train_sample.append(inputs_llava.detach().cpu()[0])
            cnt+=1
        ...
        for inputs_llava, inputs_oct, labels,_ in tqdm(valid_dataloader, position = 0, leave = True):
            # moving tensors to gpu....
            ...
            running_f1 += (f1_score(y_true = labels.detach().cpu().numpy(),
                                   y_pred  = preds.detach().cpu().numpy(),
                                   average = "binary",
                                   pos_label = 0, zero_division=0) + f1_score(y_true = labels.detach().cpu().numpy(),
                                   y_pred = preds.detach().cpu().numpy(),
                                   average = "binary",
        ...
        print(running_auc_h /len(valid_dataloader))
...
# Loading the model....
# BUILD CLIP MODEL
#processor = CLIPProcessor.from_pretrained("openai/clip-vit-large-patch14-336").image_processor
base_model = build_model(model_type = "multimodal")
...
unfreeze_list = ["llava_model.multi_modal_projector.linear_1.weight", 
                 "llava_model.multi_modal_projector.linear_1.bias",
                 "llava_model.multi_modal_projector.linear_2.weight",
                 "llava_model.multi_modal_projector.linear_2.bias",
                 "project_llava.weight",
                 "project_llava.bias",
                 "project_oct.weight",
                 "project_oct.bias",
                 "layer_2.weight",
...
base_model = base_model.to(torch.float32).to(device)
